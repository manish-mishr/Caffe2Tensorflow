...
arr = np.array( utils.blobproto_to_array(blob) )

mean = arr[0].mean(1).mean(1)
mean = mean[:,np.newaxis,np.newaxis]
mean = np.transpose(mean,(2,1,0))
print (mean.shape)
...
with tf.name_scope('fc10_merge') as scope:
    concat_first_weights = tf.concat(
        (fc8new,
    ...
    concat_second_weights = tf.concat(
        (fc8_MotionBlur,
        fc8_Object,
        fc8_Repetition,
        fc8_RuleOfThirds,
    ...
    concat_weights = tf.concat(
        (concat_first_weights,
         concat_second_weights),
        axis=1)
